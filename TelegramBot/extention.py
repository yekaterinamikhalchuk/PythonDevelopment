...
class CurrencyConverter:
    ...
    def get_price(cur_from: str, cur_to: str, amount: str):
        ...
        try:
            ...
            raise APIException(f'The currency {cur_from} is unknown. To see available currencies press "Currencies available 💸"')

        try:
            cur_to_ticker = keys[cur_to]
        except KeyError:
            raise APIException(f'The currency {cur_to} is unknown. To see available currencies press "Currencies available 💸"')
        try:
            amount = float(amount)
        except ValueError:
